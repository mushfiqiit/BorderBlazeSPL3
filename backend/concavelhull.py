...
async def concavehull():
    ...
    # Create a plot of the concave hull
    plt.scatter(points[:, 0], points[:, 1], color='blue', marker='o', label='Original Points')
    plt.plot(points[boundary_indices, 0], points[boundary_indices, 1], color='red', linestyle='-', linewidth=2, label='Concave Hull')
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
    plt.title('Concave Hull of 2D Points')
    plt.legend()
